...
namespace src
{
    public partial class frmChamDiem : Form
    {
        ...
        private void frmChamDiem_Load(object sender, EventArgs e)
        {
            ...
            double diem = 0;
            for(int i=0;i<lstBaiNop.Count;i++)
            {
                BaiNop kq = new BaiNop();
                kq = (BaiNop)(lbxBaiThi.Items[i]);
                String sLine = kq.ToString();
                String[] arrsLine = sLine.Split('/');

                BaiNop _kq = new BaiNop();
                _kq = lstDapAn[i];
                String _sLine = _kq.ToString();
                String[] arr_sLine = _sLine.Split('/');

                if(arrsLine[0] == arr_sLine[0] && arrsLine[1]==arr_sLine[1])
                {
                    diem = diem + 0.5;
                }
                txtTongDiem.Text = diem.ToString();
            }
        }
